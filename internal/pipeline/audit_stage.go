...
func (e *Executor) audit(ctx context.Context, book store.Book, r scheduler.StageReport) (scheduler.StageResult, error) {
	...
	if e.db != nil && !effectivePass {
		prev := loadAuditRounds(book.WorkDir) // BEFORE appending: every prior round
		minPrior, havePrior := minActionable(prev)
		...
		if acceptTrajectory(round, blocker, fix, minPrior, havePrior, fixesDone, valRep.Clean, state.MaxFixAttempts) {
